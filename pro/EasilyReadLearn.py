...
def lookup_sql(word):
    command="select explain, en, us, en_voice, us_voice, example from english_dictionary where word='%s'" % word
    cu.execute(command)
    return cu.fetchone()
...
def generate_word_table(word_frequence,sheet_title):
    ...
    excel_output_filename="tolearn.xlsm"
    if not os.path.isfile(excel_output_filename):
        wb.save(excel_output_filename)
    wb=openpyxl.load_workbook(filename=excel_output_filename,keep_vba=True)
    if sheet_title in wb.get_sheet_names():
        wb.remove_sheet(wb[sheet_title])
    ws=wb.create_sheet(title=sheet_title)
    ws.append([u"单词",u"生疏度",u"频率",u"解释",u"英式音标",u"美式音标",u"英式发音",u"美式发音",u"例句"])
    row_count=1;
    word_dictionary={}
    ...
    for wd in word_dictionary.keys():
        ...
        for i in range(len(word_dictionary[wd][1])):
            ws.cell(row=row_count,column=4+i).set_explicit_value(value=word_dictionary[wd][1][i],data_type="s")
...
if __name__=="__main__":
    ...
    print u"已经成功将单词表输入到了tolearn.xlsm :)"
    print u"需要去除最常用单词吗？是：Y,否：N"
    flag=raw_input()
    if flag.lower()=="y":
        os.system("python -W ignore Sync.py")
    print u"完成，可以打开tolearn.xlsm进行学习了了:)"
